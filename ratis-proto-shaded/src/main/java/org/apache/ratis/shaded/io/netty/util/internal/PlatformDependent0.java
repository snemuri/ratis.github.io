...
final class PlatformDependent0 {
    ...
    private static boolean explicitNoUnsafe0() {
        final boolean noUnsafe = SystemPropertyUtil.getBoolean("org.apache.ratis.shaded.io.netty.noUnsafe", false);
        logger.debug("-Dio.netty.noUnsafe: {}", noUnsafe);

        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.apache.ratis.shaded.io.netty.noUnsafe)");
            return true;
        }

        // Legacy properties
        boolean tryUnsafe;
        if (SystemPropertyUtil.contains("org.apache.ratis.shaded.io.netty.tryUnsafe")) {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.apache.ratis.shaded.io.netty.tryUnsafe", true);
        } else {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.jboss.netty.tryUnsafe", true);
        }

        if (!tryUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.apache.ratis.shaded.io.netty.tryUnsafe/org.jboss.netty.tryUnsafe)");
            return true;
        }
