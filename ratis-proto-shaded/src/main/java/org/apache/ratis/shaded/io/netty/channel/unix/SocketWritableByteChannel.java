...
public abstract class SocketWritableByteChannel implements WritableByteChannel {
    ...
    public final int write(java.nio.ByteBuffer src) throws java.io.IOException {
        ...
        if (src.isDirect()) {
            ...
            org.apache.ratis.shaded.io.netty.buffer.ByteBuf buffer = null;
            try {
                if (readableBytes == 0) {
                    buffer = org.apache.ratis.shaded.io.netty.buffer.Unpooled.EMPTY_BUFFER;
                } else {
                    final ByteBufAllocator alloc = alloc();
                    if (alloc.isDirectBufferPooled()) {
                        buffer = alloc.directBuffer(readableBytes);
                    } else {
                        buffer = org.apache.ratis.shaded.io.netty.buffer.ByteBufUtil.threadLocalDirectBuffer();
                        if (buffer == null) {
                            buffer = org.apache.ratis.shaded.io.netty.buffer.Unpooled.directBuffer(readableBytes);
                        }
                    }
                }
